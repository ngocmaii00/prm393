...
class Product {
  ...
  static List<Product> list = [
    Product(id: '1', name: 'Laptop Dell XPS 13', image: 'dell_xps13.png', price: 1200.0),
    Product(id: '2', name: 'iPhone 15 Pro Max', image: 'iphone15_promax.png', price: 1099.0),
    Product(id: '3', name: 'Tai nghe Sony WH-1000XM5', image: 'sony_wh1000.png', price: 349.9),
    Product(id: '4', name: 'Chuột Logitech MX Master 3S', image: 'logi_mx3s.png', price: 99.0),
    Product(id: '5', name: 'Bàn phím Keychron K2', image: 'keychron_k2.png', price: 79.5),
  ];
...
void displayList(List<Product> products) {
  ...
  print('\n-------------------------------------------------------------------------------------');
  print('                        DANH SÁCH SẢN PHẨM (${products.length}/${Product.maxLimit})');
  print('-------------------------------------------------------------------------------------');
  print('ID    | Tên sản phẩm                   | Giá (USD)    | Link hình ảnh');
  print('-------------------------------------------------------------------------------------');
  for (var p in products) {
    final idStr = p.id.padRight(5);
    final nameStr = (p.name.length > 30 ? p.name.substring(0, 27) + '...' : p.name).padRight(30);
    final priceStr = '\$${p.price.toStringAsFixed(2)}'.padRight(12);
    final imageStr = p.image.length > 25 ? p.image.substring(0, 22) + '...' : p.image;
...
void handleAddProduct() {
  if (Product.list.length >= Product.maxLimit) {
    ...
    print('Số lượng sản phẩm hiện tại (${Product.list.length}) đã đạt tối đa (${Product.maxLimit}).');
    return;
  }
...
void handleAddProductFromJson() {
  ...
  print('Ví dụ chuỗi JSON hợp lệ: {"id": "6", "name": "Bàn di chuột", "image": "mousepad.png", "price": 25.5}');
  ...
  try {
    ...
    if (!jsonMap.containsKey('id') || jsonMap['id'] == null || jsonMap['id'].toString().trim().isEmpty) {
...
void handleEditProduct() {
  ...
  stdout.write('Nhập giá mới (để trống để giữ nguyên "\$${old.price.toStringAsFixed(2)}"): ');
  String priceInput = stdin.readLineSync(encoding: utf8)?.trim() ?? '';
...
void handleDeleteProduct() {
  ...
  Product old = Product.list[index];
  stdout.write('Bạn có chắc chắn muốn xóa sản phẩm "${old.name}" không? (y/n): ');
  String confirm = stdin.readLineSync(encoding: utf8)?.trim().toLowerCase() ?? '';
...
void handleFilterProducts() {
  ...
  switch (choice) {
    ...
    case '2':
      ...
      String name = stdin.readLineSync(encoding: utf8)?.trim() ?? '';
      List<Product> results = Product.list.where((p) => p.name.toLowerCase().contains(name.toLowerCase())).toList();
      displayList(results);
      break;
    ...
    case '4':
      ...
      List<Product> results = Product.list.where((p) {
        ...
        if (nameFilter.isNotEmpty && !p.name.toLowerCase().contains(nameFilter.toLowerCase())) return false;
...
void handleSortProducts() {
  ...
  if (criteria == '1') {
    Product.list.sort((a, b) => asc ? a.name.compareTo(b.name) : b.name.compareTo(a.name));
  } else {
    Product.list.sort((a, b) => asc ? a.price.compareTo(b.price) : b.price.compareTo(a.price));
  }
